...
Layer::Layer(Image *image, int zindex) : GameObject(zindex), image(image), extractionPosition(0, 0) {
}

Layer::Layer(Image *image, Vector2 position, int zindex) : GameObject(position, zindex), image(image),
                                                           extractionPosition(0, 0) {
}
...
void Layer::plotTo(Image *destination) {
    this->image->extractAndPlotTo(destination, this->position, this->extractionPosition,
                                  Vector2(destination->getWidth(), this->image->getHeight()));
}
