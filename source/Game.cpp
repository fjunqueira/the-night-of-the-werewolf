...
void gameover(void) {
    ...
    displayText((float) -0.15, 0.7, 1, 0, 0, pointsText);
...
#ifdef  _WIN32
void CALLBACK addEnemyHandler(HWND hwnd, UINT uMsg, UINT timerId, DWORD dwTime){
    addEnemy();
}
#endif
...
void addEnemyHandler(int signum) {
    addEnemy();
}
...
void initGameData() {
    ...
    player = new Player(Animation(8, runningSprites), Animation(7, jumpingSprites),
                        Vector2(background->getWidth() / 2 - 100, 85), 5);
